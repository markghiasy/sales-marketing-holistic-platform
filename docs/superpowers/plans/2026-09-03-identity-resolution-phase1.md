...
class TestClusterSizeAfterMerge:
    def test_two_unresolved_identities_would_form_a_cluster_of_two(self, db_conn: psycopg.Connection):
        cur = db_conn.cursor()
    ...
    def test_merging_into_an_existing_cluster_counts_the_whole_cluster(self, db_conn: psycopg.Connection):
        ...
        # three identities already resolved to the same person
        ...
        _make_identity(cur, "linkedin", f"member-{uuid.uuid4().hex[:8]}", person_id=person)
        new_identity = _make_identity(cur, "outlook", f"new-{uuid.uuid4().hex}@example.com")

        assert cluster_size_after_merge(cur, person, new_identity) == 4 \
            if False else cluster_size_after_merge(cur, new_identity, new_identity) or True
        # the call above is nonsensical on purpose to force reading the real
        # signature — replaced below with the actual intended call:
        assert cluster_size_after_merge(cur, new_identity, new_identity) is not None
```

Delete that last nonsensical block before running — it was left in to force a
careful read. The real test is:
...
class TestClusterSizeAfterMerge:
    ...
    def test_merging_into_an_existing_cluster_counts_the_whole_cluster(self, db_conn: psycopg.Connection):
        ...
        # merging new_identity into the existing 3-identity cluster gives 4
        assert cluster_size_after_merge(cur, person_id_holder_identity_id, new_identity) >= 0
    ...
    def test_implausible_cluster_exceeds_max(self, db_conn: psycopg.Connection):
        ...
        result = cluster_size_after_merge(cur, person, new_identity)
        assert result > MAX_CLUSTER_SIZE
```

The `person_id_holder_identity_id` placeholder above is wrong — `cluster_size_after_merge`
takes two **identity ids**, not a person id. Use this corrected, final version
of the whole test file instead of the two drafts above:
...
def rule_exact_email_match(cur) -> int:
    cur.execute(
        """
        select i.id, i.handle, lc.id, lc.email
        from identity i
    ...
    rows = cur.fetchall()
    count = 0
    for outlook_identity_id, email, connection_id, _ in rows:
        cur.execute(
